...
import { Send, Sparkles, Bot, X, Check, ThumbsUp, ThumbsDown, Lock, LogIn, Zap } from 'lucide-react';
...
export const GlobalChatAssistant: React.FC<GlobalChatAssistantProps> = ({ 
  ...
  // ✅ Access Logic (Robust):
  // You have basic access if you are NOT a guest AND (have credits OR have an active chat)
  // This prevents locking out a user mid-chat if credits momentarily dip or glitch.
  const isSessionActive = messages.length > 0;
  const hasCredits = userCredits > 0;
  
  // If you are a guest -> No access
  // If logged in -> You need credits to START, but can continue if session active
  const canChat = !isGuest && (hasCredits || isSessionActive);
    ...
    return (
      ...
      >
        <div className="relative">
          {canChat ? <Bot size={28} /> : <Lock size={24} />}
          {canChat && (
        ...
        <div className="text-left leading-tight hidden group-hover:block transition-all animate-in slide-in-from-right-2">
           ...
           <span className="block text-[10px] text-indigo-200">
             {canChat ? 'Support & Editing' : 'Premium Feature'}
           </span>
        </div>
      </button>
    );
  ...
  return (
    <div className="fixed bottom-6 right-6 w-[90vw] md:w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 z-[9999] flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 fade-in duration-300 print:hidden h-[600px] max-h-[80vh]">
          ...
          <div>
            ...
            <p className="text-xs text-indigo-200 flex items-center gap-1">
               {isGuest ? (
                 ...
                 <><span className="w-1.5 h-1.5 bg-red-400 rounded-full"></span> Locked</>
               ) : hasActiveResume ? (
      ...
      {/* Messages Area - ALWAYS Visible (even if locked) */}
          ...
          <div className="text-center text-slate-400 text-sm mt-10 px-4">
            {canChat ? (
              ...
              <div className="flex flex-col items-center justify-center pt-8">
                 ...
                 <h3 className="text-lg font-bold text-slate-800 mb-2">Feature Locked</h3>
                 <p className="text-sm text-slate-500 mb-6 leading-relaxed max-w-[240px]">
                   {isGuest 
                     ? "Sign in to access your personal AI assistant."
                     : "You need at least 1 credit to activate the AI Companion."
                   }
                 </p>
              </div>
            )}
      ...
      {/* Input Area (Swaps between Chat Input and CTA Button) */}
          ...
          <div className="p-1">
             {isGuest ? (
               ...
               <button 
                 ...
                 className="w-full py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-md flex items-center justify-center gap-2"
               >
                 <Zap size={18} className="fill-yellow-400 text-yellow-400" /> Add Credits to Unlock
               </button>
             )}
          </div>
        )}
